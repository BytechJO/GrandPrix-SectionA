...
import './Page18_Q5.css'
import { NONE } from "@jsplumb/browser-ui";

const images = [
  { id: 'a', src: img1, label:'elle s’appelle Anabelle.' },
  { id: 'b', src: img2, label: 'ils s’appellent Paul et Thomas.' },
  { id: 'c', src: img3, label: 'il s’appelle Boris.' },
  { id: 'd', src: img4, label: 'elles s’appellent Sophia et Alison.' },
];

const correctAnswers = {
  a: 'elle s’appelle Anabelle.',
  b: 'ils s’appellent Paul et Thomas.',
  c: 'il s’appelle Boris.',
  d: 'elles s’appellent Sophia et Alison.',
};
...
const getInputClass = (id) => {
  if (!score) return ""; // قبل التحقق لا يوجد لون
  return (answers[id] || '').toLowerCase() === correctAnswers[id].toLowerCase()
    ? "correct-input"
    : "wrong-input";
};
...
className="header-title-page1 w-full text-left mb-4"
  style={{ marginLeft: "42%", color:"black",marginTop:"5%",fontSize:"25px", fontWeight:"bold" }}
      >
        <span style={{backgroundColor:"#73C8D2"}} className="ex-A">D</span> <span style={{color:"black"}} className="number-of-q">5</span> Associe les noms aux photos. Utilise leverbe « s’appeler ».
      </header>
...
{score && <ScoreCardEnhanced score={score} />}
      ...
      <div className="exercise-images grid grid-cols-2 gap-6">
        {images.map(img => (
          <div key={img.id} className="flex flex-col items-center gap-2">
            <img src={img.src} alt={`Image ${img.label}`} className="w-32 h-32 object-contain"/>
       <input
  type="text"
  maxLength="30"
  placeholder=""
  ...
  className={`border rounded p-1 w-16 text-center ${getInputClass(img.id)}`}
  style={{border:"none",borderBottom:"2px solid" , borderRadius:"0",width:"70%"}}
/>
